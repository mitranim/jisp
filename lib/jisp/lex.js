(function() {
  ...
  function demand(tokens, args) {
    var conditions, modes, condition, mode, test, _i, _res, _ref;
    args = 2 <= arguments.length ? [].slice.call(arguments, 1, _i = arguments.length - 0) : (_i = _i, []);
    (conditions = []);
    (modes = []);
    _res = [];
    while ((args.length > 0)) {
  ...
  function lex(tokens, mode) {
    ...
    switch (mode) {
      ...
      case "unquote":
        demand(tokens, ",", "drop");
        _ref0 = ["unquote", addProperties(tokens, demand(tokens, "(", "list", "`", "quote", isIdentifier, "atom"))];
        break;
      case "spread":
